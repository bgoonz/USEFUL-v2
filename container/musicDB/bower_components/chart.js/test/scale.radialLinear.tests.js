...
describe("Test the radial linear scale", function () {
	...
	it("Should build labels using the user supplied callback", function () {
		...
		expect(chart.scale.pointLabels).toEqual([
			"label1",
			"label2",
			"label3",
			"label4",
	...
	it("should get the correct distance from the center point", function () {
		...
		expect(chart.scale.getDistanceFromCenterForValue(chart.scale.min)).toBe(
			0
		);
		expect(chart.scale.getDistanceFromCenterForValue(chart.scale.max)).toBe(
			233
		);
	...
	it("should correctly get angles for all points", function () {
		...
		for (var i = 0; i < 5; i++) {
			expect(radToNearestDegree(chart.scale.getIndexAngle(i))).toBe(
				15 + slice * i
			);
		}
		...
		for (var x = 0; x < 5; x++) {
			expect(radToNearestDegree(chart.scale.getIndexAngle(x))).toBe(
				slice * x
			);
		}
	});
