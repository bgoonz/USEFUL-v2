describe("Chart.Controller", function () {
	function waitForResize(chart, callback) {
		...
		Chart.helpers.addEvent(
			content,
			state !== "complete" ? "load" : "resize",
			handler
		);
	}
	...
	describe("config initialization", function () {
		...
		it("should initialize config with default options", function () {
			...
			expect(options.defaultFontSize).toBe(
	...
	describe("config update", function () {
		...
		it("should update tooltip options", function () {
			...
			chart.update();
			expect(chart.tooltip._options).toEqual(
				jasmine.objectContaining(newTooltipConfig)
			);
		});
	});
