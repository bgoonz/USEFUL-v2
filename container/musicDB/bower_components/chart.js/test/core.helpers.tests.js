describe("Core helper tests", function () {
	...
	it("should return the width of the longest text in an Array and 2D Array", function () {
		...
		expect(helpers.longestText(context, font, arrayOfThings1D, {})).toEqual(
			60
		);
		expect(helpers.longestText(context, font, arrayOfThings2D, {})).toEqual(
			80
		);
	...
	it("compare text with current longest and update", function () {
		...
		expect(
			helpers.measureText(context, data, gc, longest, "foobar")
		).toEqual(70);
		expect(
			helpers.measureText(context, data, gc, longest, "foobar_")
		).toEqual(70);
		expect(
			helpers.measureText(context, data, gc, longest, "foobar_1")
		).toEqual(80);
		// We check to make sure we made the right calls to the canvas.
		expect(context.getCalls()).toEqual([
			{
