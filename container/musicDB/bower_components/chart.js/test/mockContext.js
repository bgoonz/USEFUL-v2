...
(function () {
			...
			compare: function (actual) {
				...
				if (!(actual instanceof Chart.Controller)) {
					message =
						"Expected " +
						actual +
						" to be an instance of Chart.Controller";
					...
					message =
						"Expected canvas to be an instance of HTMLCanvasElement";
					...
					!isFinite(chart.height)
				) {
					message = "Expected height to be a strict finite number";
				} else if (
					typeof chart.width !== "number" ||
					!isFinite(chart.width)
				) {
	...
	function acquireChart(config, options) {
		...
		config.options.responsive =
			config.options.responsive === undefined
				? false
				: config.options.responsive;
