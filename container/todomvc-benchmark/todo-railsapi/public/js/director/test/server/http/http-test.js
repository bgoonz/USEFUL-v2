      ...
      "instantiated with a Routing table": {
        ...
        "when passed to an http.Server instance": {
          ...
          "a request to add/:param/bark": {
            topic: function () {
              request(
                { uri: "http://localhost:9090/add/test/bark" },
                this.callback
              );
            },
          ...
          "an encoded request to add/:param/bark": {
            topic: function () {
              request(
                {
                  uri: "http://localhost:9090/add/something%40example.com/bark",
                },
                this.callback
              );
            },
          ...
          "an encoded request to add/:param/bark with additional special characters":
            {
              topic: function () {
                request(
                  {
                    uri: "http://localhost:9090/add/something%40(example).com/bark",
                  },
                  this.callback
                );
              },
