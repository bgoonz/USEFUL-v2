...
const hasuraFetch = (user, projectConfig) => {
  return (
    ...
    if (!projectConfig.baseDomain) {
      logError(
        "Please use hasura.setProject or hasura.setBaseDomain before making any API calls."
      );
      return;
    }
    ...
    fetch(url, {
      ...
      (response) => {
        if (response.status >= 200 && response.status < 300) {
          ...
          logError(
            "Redirect response received. Are you sure you're querying the right endpoint?"
          );
          emitError(options, response);
          ...
          logError(
            "Unhandled error in Hasura SDK. Please file an issue at github.com/hasura/support with the information below:"
          );
          emitError(options, response);
          onException();
        }
      ...
      (error) => {
        logError(
          "Failed to make request. Check http/https, service name, base domain or path values used?\n>>>"
        );
        logError(options);
        logError("<<<");
        logError(error);
